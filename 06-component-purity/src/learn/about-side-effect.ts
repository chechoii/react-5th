...
function greet(name:string):string{
    console.log(`Hello ${name}!`); // 내가 해야 하는 일 이외의 일
    return `Hello ${name}!`
}
...
// 출력 결과는 같음, but 내가 해야 하는 일 이외의 다른 일까지 처리 => 불순한 함수
// 콘솔에 출력이 발생 => side Effect 발생!! (부수 효과 발생)
...
브라우저 API호출 (setTimeout, setInterval)
로컬 스토리지 접근
